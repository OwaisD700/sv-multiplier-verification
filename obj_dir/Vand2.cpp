// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Model implementation (design independent parts)

#include "Vand2__pch.h"

//============================================================
// Constructors

Vand2::Vand2(VerilatedContext* _vcontextp__, const char* _vcname__)
    : VerilatedModel{*_vcontextp__}
    , vlSymsp{new Vand2__Syms(contextp(), _vcname__, this)}
    , a{vlSymsp->TOP.a}
    , b{vlSymsp->TOP.b}
    , y{vlSymsp->TOP.y}
    , rootp{&(vlSymsp->TOP)}
{
    // Register model with the context
    contextp()->addModel(this);
}

Vand2::Vand2(const char* _vcname__)
    : Vand2(Verilated::threadContextp(), _vcname__)
{
}

//============================================================
// Destructor

Vand2::~Vand2() {
    delete vlSymsp;
}

//============================================================
// Evaluation function

#ifdef VL_DEBUG
void Vand2___024root___eval_debug_assertions(Vand2___024root* vlSelf);
#endif  // VL_DEBUG
void Vand2___024root___eval_static(Vand2___024root* vlSelf);
void Vand2___024root___eval_initial(Vand2___024root* vlSelf);
void Vand2___024root___eval_settle(Vand2___024root* vlSelf);
void Vand2___024root___eval(Vand2___024root* vlSelf);

void Vand2::eval_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+++++TOP Evaluate Vand2::eval_step\n"); );
#ifdef VL_DEBUG
    // Debug assertions
    Vand2___024root___eval_debug_assertions(&(vlSymsp->TOP));
#endif  // VL_DEBUG
    vlSymsp->__Vm_deleter.deleteAll();
    if (VL_UNLIKELY(!vlSymsp->__Vm_didInit)) {
        VL_DEBUG_IF(VL_DBG_MSGF("+ Initial\n"););
        Vand2___024root___eval_static(&(vlSymsp->TOP));
        Vand2___024root___eval_initial(&(vlSymsp->TOP));
        Vand2___024root___eval_settle(&(vlSymsp->TOP));
        vlSymsp->__Vm_didInit = true;
    }
    VL_DEBUG_IF(VL_DBG_MSGF("+ Eval\n"););
    Vand2___024root___eval(&(vlSymsp->TOP));
    // Evaluate cleanup
    Verilated::endOfEval(vlSymsp->__Vm_evalMsgQp);
}

//============================================================
// Events and timing
bool Vand2::eventsPending() { return false; }

uint64_t Vand2::nextTimeSlot() {
    VL_FATAL_MT(__FILE__, __LINE__, "", "No delays in the design");
    return 0;
}

//============================================================
// Utilities

const char* Vand2::name() const {
    return vlSymsp->name();
}

//============================================================
// Invoke final blocks

void Vand2___024root___eval_final(Vand2___024root* vlSelf);

VL_ATTR_COLD void Vand2::final() {
    contextp()->executingFinal(true);
    Vand2___024root___eval_final(&(vlSymsp->TOP));
    contextp()->executingFinal(false);
}

//============================================================
// Implementations of abstract methods from VerilatedModel

const char* Vand2::hierName() const { return vlSymsp->name(); }
const char* Vand2::modelName() const { return "Vand2"; }
unsigned Vand2::threads() const { return 1; }
void Vand2::prepareClone() const { contextp()->prepareClone(); }
void Vand2::atClone() const {
    contextp()->threadPoolpOnClone();
}
